...
def deleteUser(request):
     ...
     ename = EmpData.objects.get(request.POST['chk'])
...
def addUser(request):
       ...
       empData=EmpData(ename=ename,eemail=eemail)
       empData.save()
...
def serachKeyword(request):
    ...
    searchResult = elasticSearchObj.search(index='questionnaire',
                                           size = 9999,
                                           body={
                                               "query":{
                                                   "match":{
    ...
    return render(request,'djangodemo/homepage.html',{'result':requiredData,'nameSet':venderNameset,'keyword':keyword})
...
def indexFile(request):
    ...
    elasticSearchObj.indices.create(index='questionnaire', ignore=400)
    for csvFile in request.FILES.getlist('csvfile'):
        ...
        indexCounter = 1
        for row in reader:
            elasticSearchObj.index(index='questionnaire', doc_type='document', id=str(csvFile) + str(indexCounter),
                                      body=row)  # indexing file
            indexCounter += 1

        try:
            while True:
                elasticSearchObj.delete(index='questionnaire', doc_type='document', id=str(csvFile) + str(indexCounter))
                indexCounter += 1
        except:
            pass
