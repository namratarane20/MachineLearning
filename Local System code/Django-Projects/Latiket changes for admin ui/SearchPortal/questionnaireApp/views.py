...
def fillQuestionnaire(request, clientId):
    ...
    if request.method == 'GET':
        if request.GET.get('name') == 'final':
            ...
            newQuestionnaireDataValues = QuestionDB.objects.filter(admin__clientId=clientId).values()
            vendorName = AdminDB.objects.filter(clientId=clientId)[0].vendorName
            newQuestionnaireData1 = list(newQuestionnaireDataValues)
        ...
        if request.POST['save'] == 'save':
            for object in QuestionDB.objects.filter(admin__clientId=clientId):
                responseKey = re.sub(r"\W", "", (object.question + str(object.id) + "response"))
                commentKey = re.sub(r"\W", "", (object.question + str(object.id) + "comment"))
                print ("response : ", request.POST[responseKey])
...
# def slackNotification(request,slack_user_name,question):
#     slack_user_list = request.POST.getlist("slackUserName[]")
#     print("slack user id ",slack_user_list)
#     print("slack user name",slack_user_name)
#     print("question data---->>",urllib.parse.unquote(question))
#     print("client_id",globalClientId)
#     assignToObj = AssignTo.objects.filter(question_db=urllib.parse.unquote(question),assignTo=slack_user_name)
#     if assignToObj:
#         print("data presend")
#     else:
#         ToObj=AssignTo(question_db=urllib.parse.unquote(question),assignTo=slack_user_name)
#         ToObj.save()
