...
def indexQuestionnaireFile(request):
    ...
    for csvFile in request.FILES.getlist('questionnaireFile'):
        ...
        indexCounter = 1
        for row in reader:
            row['clientId'] = row['VendorName'] + " " + request.POST['questionnaireSubmittedDate']
...
def fillQuestionnaire(request, clientId):
    fileName = ''
    systemPath = ''
    dowloadDataFrame = pd.DataFrame(columns=['Security Questions','Response','Additional Comment'])
    ...
    if request.method == 'GET':
        if request.GET.get('name') == 'final':
            ...
            for hit in questionnaireSearchResult['hits']['hits']:
                if hit['_source']['clientId'] == clientId:
                    questionnaireRequiredData.append(hit['_source'])  # extracting required data
            context = {"vendorName": vendorName, "vendorsData": questionnaireRequiredData, "dataFromElasticsearch":True}
        else:
            ...
            context = {"dataFromDatabase": True, 'vendorsData': newQuestionnaireData, "vendorName": vendorName}

    else:
        dataForDownload = QuestionDB.objects.filter(admin__clientId=clientId).values()
        listOfDataForDownload = list(dataForDownload)
        print('data for download csv', listOfDataForDownload)
        questionList = []
        responsesList = []
        commentList = []
        for downloadData in listOfDataForDownload:
            ...
            # dowloadDataFrame['question'] = questionList
            # dowloadDataFrame['response'] =responsesList
            # dowloadDataFrame['additionalComment']= commentList
            dowloadDataFrame = pd.DataFrame(
                {'Security Questions': questionList, 'Response': responsesList, 'Additional Comment': commentList})
        # print('question lis ===========',questionList)
        # print('resopmeList =========',responsesList)
        # print('comment lis==========',commentList)
        print('mmy dataframe =======', dowloadDataFrame)
        ...
        print('Myadmin db list',listOfAdminDB)
        for fileNameForDownload in listOfAdminDB:
            ...
            # print('my path', systemPath)


        try:
            if request.POST['csvDownload'] == 'csvDownload':

                dowloadDataFrame.to_csv(systemPath + fileName)
                print('CSV FILE DOWNLODED SUCCESSFULLY=============')
                newQuestionnaireData = QuestionDB.objects.filter(admin__clientId=clientId)
                vendorName = AdminDB.objects.filter(clientId=clientId)[0].vendorName
                context = {"dataFromDatabase": True, 'vendorsData': newQuestionnaireData, "vendorName": vendorName}
                messages.success(request,"CSV File downloded successfully !!!!! " )

        except:
            if request.POST['excelDownload'] == 'excelDownload':
                removedFileName = fileName[:-4]
                # print('my file name without extension',removedFileName)
                dowloadDataFrame.to_excel(systemPath + removedFileName + '.xlsx')
                # print('my path', systemPath)
                print('EXCEL  FILE DOWNLODED SUCCESSFULLY=============')
                newQuestionnaireData = QuestionDB.objects.filter(admin__clientId=clientId)
                vendorName = AdminDB.objects.filter(clientId=clientId)[0].vendorName
                context = {"dataFromDatabase": True, 'vendorsData': newQuestionnaireData, "vendorName": vendorName, }
                messages.success(request,"Excel File downloded successfully !!!!! " )
...
def downloadCsv(request):
    return fillQuestionnaire(request)
...
def uploadNewQuestionnaireToDatabase(request):
    for csvFile in request.FILES.getlist('questionnaireFile'):
        ...
        for row in reader:
            QuestionDB(admin=adminDbObject, question=row['SecurityQuestions'], response=row['Responses'],
                       additionalComment=row['AdditionalComments']).save()
