...
def addToTags(request, str=None):
    ...
    # below commented logic is to sort data for new questionnaire and old questionnaire
    # newVendorsList = []
    # oldVendorsList = []
...
def indexQuestionnaireFile(request):
    ...
    for csvFile in request.FILES.getlist('questionnaireFile'):
        ...
        if request.POST['questionnaireSubmittedDate'] == '':
            submittedDate = datetime.today().strftime('%Y-%m-%d')
        else:
        ...
        try:
            ...
            adminDbObject.clientId = vendorName + " " + submittedDate
            ...
            AdminDB(fileName=str(csvFile), vendorName=vendorName, submittedDate=submittedDate, status=1,
                    ...
                    clientId=(vendorName + " " + submittedDate), summary=scope, documentLink=docLink).save()
