...
str=""
def searchKeyword(request,recentKeyword=None):
    print("inside search keyword")
    tagListWithFlag = TagWithInfo.objects.filter(flag=1).values()
    ...
    for i, hit in enumerate(policiesResultList['hits']['hits']):
        ...
        print("==============================================================>",policiesSearchResult[i]['Data'])
    ...
    if (getSearchedKeyword.casefold() == 'questionnaire'):
        context = {'questionnaireResult': questionnaireRequiredData,
               'questionnaireVendorNameSet': questionnnaireVendorNameSet,'questionnaireId': questionnaireRequiredId, 'quelist1': queList, "tagNameList":tagNameList,'keyword':keyword,'active': 'active'}
    elif (getSearchedKeyword.casefold() == 'policies'):
        context = {'questionnaireVendorNameSet': questionnnaireVendorNameSet, 'policiesResult': policiesSearchResult,
               'questionnaireId': questionnaireRequiredId, 'quelist1': queList, "tagNameList":tagNameList,'keyword':keyword, 'active': 'active'}
    else:
        context = {'questionnaireResult': questionnaireRequiredData,
...
def addToTags(request):
    ...
    try:
        ...
        #
        # allTagNameColoumn = TagWithInfo.objects.filter(flag=1).values()
        # allData = list(allTagNameColoumn)
        # for data in allData:
        #    tagnames= data['tagName']
        #    print('mt tag name ++++++++++++',tagnames)
        #    usernames = data['userName']
        #    print('myuser name +++++++++++',usernames)
        #    tagNameAndUsernameDict.update({tagnames:usernames})
        #
        # print('mydict is +++++++========',tagNameAndUsernameDict)
        # print('-----------------------',type(tagNameAndUsernameDict))


        return render(request, 'questionnaireApp/demo.html', {'dateUserTagDict':dateUserTagDict,'newSearchTable':newSearchTable,'order':order})
