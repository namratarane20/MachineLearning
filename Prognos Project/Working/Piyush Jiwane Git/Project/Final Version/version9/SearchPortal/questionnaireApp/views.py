...
def addToTags(request,str=None):
    ...
    for u in usernameList:
        ...
        print('-------------->>Color Code<<-----------', hex_number)
        for key,value in userDict.items():
            if key == u:
                u = u.replace(" ", "-")
                dict1.update({u: {'alias': value, 'colorCode': hex_number, 'keyData': keywordList2} })
    print(dict1)
