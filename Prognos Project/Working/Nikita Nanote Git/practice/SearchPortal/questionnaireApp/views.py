...
def searchKeyword(request):
    ...
    if check_list == ['1']:
        ...
        return render(request, 'questionnaireApp/home.html', {'result': requiredData, 'nameSet': vendorNameSet,'question':question})

    if check_list==['2']:
        policy = True
        return HttpResponse("Policies Data is not available")
    ...
    if check_list==['1','2']:
        ...
        for hit in searchResult['hits']['hits']:
            requiredData.append(hit['_source'])  # extracting required data
            vendorNameSet.add(hit['_source']['VendorName'])
        return render(request,'questionnaireApp/home.html',{'result': requiredData, 'nameSet': vendorNameSet,'both': both})
