...
def searchKeyword(request, recentSearch=None):
    ...
    if request.POST:
        strKeyword = request.POST['keyword']
    else:
        strKeyword = recentSearch
    ...
    print("=============================>", strKeyword)
    getSearchedKeyword = strKeyword.split(' ', 1)[0]
    print("=============================>", getSearchedKeyword)
    if getSearchedKeyword in inSearchKeyword:
        keyword = strKeyword.split(' ', 1)[1]
    else:
        keyword = strKeyword
    ...
    #        }}
    #
    # res = elasticSearchObject.search(index='question_naire', body=doc, scroll='1m')
    # for doc in res['hits']['hits']:
    #     allData.append(doc['_source']['KeywordSearched'])
    # listOfNums = list(list(allData))
    # for i in listOfNums:
    #     if i != '':
    #         if j in i:
    #             str1 = str1 + j
...
def mostRelevant(request, keyword1):
    return searchKeyword(request, keyword1 )
...
def addToTags(request):
    # context = tagCreation(request)
    # return render(request, 'questionnaireApp/index.html', context)

    # Logit for Most Recent amd Most Search Keyword
    global globalRequiredData
    global strKeyword
    key_count = 0
    if strKeyword:
