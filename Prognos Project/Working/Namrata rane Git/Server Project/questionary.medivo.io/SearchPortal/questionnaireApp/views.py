...
def searchKeyword(request, recentSearch=None, relevantSearch=None):
    ...
    if questionnaireSearchResult['hits']['hits']:
        ...
        # import json
        # jsonData = json.dumps(tagAndDescDict)

        print('this is my tagname and tagDescription dict',tagAndDescDict, type(tagAndDescDict))
        ...
        if getSearchedKeyword.casefold() == 'questionnaire':
            context = {'questionnaireResult': globalRequiredData, 'questionnaireId': questionnaireRequiredId,
                       ...
                       'userKeyword': userKeyword.lower(),'tagAndDescDict':tagAndDescDict}
            ...
            context = {'questionnaireResult': globalRequiredData, 'policiesResult': policiesSearchResult,'tagAndDescDict':tagAndDescDict,
...
def addToTags(request, str=None):
    ...
    try:
        ...
        # print('this is current selected tag data', tagsList)
        totalCount =0
        duplicateCount= 0
        for data in tagsList:
            ...
            userName = request.user.get_full_name()
            dataBase = TagWithInfo.objects.all().values()
            ...
            for addToDb in addTodatabaseList:
                if col1 == addToDb['vendorname'] and col2 == addToDb.get('sectionname') and col3 == addToDb.get(
                        ...
                        'response') and col6 == addToDb.get(
                        'additionalcomment') and tagNameWithOutSpaces == addToDb.get('tagName'):
                    print("similar data, already tag exists")
                    duplicateCount += 1
                    print('dupliacte row count ',duplicateCount)
                    flag = True
                    break
            if not flag:
                ...
                print('data saved')
                totalCount += 1
        total = totalCount+ duplicateCount
        print('this is duplicate count ', duplicateCount)
        print('total slected row',total)
